<?php
$sortLabels = ['default' => 'Varsayılan', 'newest' => 'En yeni', 'price_asc' => 'Fiyat (artan)', 'price_desc' => 'Fiyat (azalan)', 'name_asc' => 'İsim (A-Z)'];
...
<h1 style="margin: 0 0 0.5rem; font-size: 1.5rem;"><?= htmlspecialchars($category['name']) ?></h1>
<?php if (!empty($category['description'])): ?>
    <p style="color: #666; margin-bottom: 1rem;"><?= nl2br(htmlspecialchars($category['description'])) ?></p>
<?php endif; ?>

<?php if (empty($products)): ?>
    <p style="color: #666;">Bu kategoride henüz ürün yok.</p>
<?php else: ?>
    <p style="margin: 0 0 1rem; color: #666;"><?= (int) $totalRows ?> ürün.</p>

    <form method="get" action="<?= htmlspecialchars($baseUrl) ?>/kategori/<?= htmlspecialchars($categorySlug) ?>" style="display: flex; flex-wrap: wrap; align-items: center; gap: 0.75rem; margin-bottom: 1.5rem;">
    ...
    <?php if ($totalPages > 1): ?>
        <nav style="margin-top: 2rem; display: flex; flex-wrap: wrap; gap: 0.5rem; justify-content: center;">
            <?php for ($i = 1; $i <= $totalPages; $i++): ?>
                ...
                <?php else: ?>
                    <a href="<?= $url ?>" style="padding: 0.5rem 0.75rem; background: #fff; border: 1px solid #ddd; border-radius: 4px; text-decoration: none; color: #333;"><?= $i ?></a>
                <?php endif; ?>
            <?php endfor; ?>
        </nav>
    <?php endif; ?>
